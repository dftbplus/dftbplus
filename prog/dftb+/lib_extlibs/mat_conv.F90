...
module mat_conv
  ...
  !> Real matrix in DFTB+ internal sparse format
  type TDftbMat_r

    !> The sparse matrix to convert
    real(dp), pointer :: sparse(:)

    !> Orbitals per atom
    integer, pointer :: nOrb(:)

    !> Starting positions of the atoms
    integer, pointer :: iSquare(:)

    !> Starting position of atom-neighbor
    ...
    !> Number of neighbors
    integer, pointer :: nNeighbor(:)

    !> Image in the central cell
    integer, pointer :: img2CentCell(:)

    !> Angular momentum of each atom
    integer, pointer :: mAngAtom(:)
