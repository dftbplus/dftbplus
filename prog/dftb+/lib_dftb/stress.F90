...
module dftbp_stress
  ...
  public :: getHelicalRepLatForce, getHelicalNonSCCLatForce, getHelicalBlockLatForce
...
contains
  ...
  !> The helical boundary condition derivatives of the repulsive energy
  subroutine getHelicalRepLatForce(deriv, coords, nNeighbours, iNeighbours, species, img2CentCell,&
      & repCont, latVecs)

    !> derivative components
    real(dp), intent(out) :: deriv(2)
    ...
    do iAt1 = 1, size(nNeighbours)
      do iNeigh = 1, nNeighbours(iAt1)
        ...
        deriv(2) = deriv(2) - prefac * ( -sin(theta)*intermed(1) + cos(theta)*intermed(2) ) &
            & * rad * nTurns
      end do
    end do

  end subroutine getHelicalRepLatForce

  
  !> The helical boundary condition derivatives from the non-SCC model
  subroutine getHelicalNonSCCLatForce(deriv, derivator, DM, EDM, skHamCont, skOverCont, coords,&
      & species, iNeighbour, nNeighbour, img2CentCell, iPair, orb, latvecs)
    ...
    integer   :: iOrig, ii, jj
    integer   :: nAtom, iNeigh, iAt1, iAt2, iAt2f
    integer   :: nOrb1, nOrb2
    real(dp)  :: sqrDMTmp(orb%mOrb,orb%mOrb), sqrEDMTmp(orb%mOrb,orb%mOrb)
    real(dp)  :: hPrimeTmp(orb%mOrb,orb%mOrb,3), sPrimeTmp(orb%mOrb,orb%mOrb,3)
  ...
  end subroutine getHelicalNonSCCLatForce


  !> The helical boundary condition derivative contributions from a potential
  subroutine getHelicalBlockLatForce(deriv, derivator, DM, EDM, skHamCont, skOverCont, coords,&
      & species, iNeighbour, nNeighbour, img2CentCell, iPair, orb, shift, latvecs)
    ...
    real(dp)  :: hPrimeTmp(orb%mOrb,orb%mOrb,3), sPrimeTmp(orb%mOrb,orb%mOrb,3)
    real(dp)  :: shiftSprime(orb%mOrb,orb%mOrb)
    real(dp)  :: vect(3), intermed, prefac, nTurns
    ...
    do iAt1 = 1, nAtom
      ...
      do iNeigh = 1, nNeighbour(iAt1)
        ...
        sPrimeTmp(:,:,:) = 0.0_dp
        sqrDMTmp(1:nOrb2,1:nOrb1) = &
            & reshape(DM(iOrig+1:iOrig+nOrb1*nOrb2,1), (/nOrb2,nOrb1/))
        sqrEDMTmp(1:nOrb2,1:nOrb1) = &
            & reshape(EDM(iOrig+1:iOrig+nOrb1*nOrb2), (/nOrb2,nOrb1/))
        call derivator%getFirstDeriv(hPrimeTmp, skHamCont, coords, species, iAt1, iAt2, orb)
        call derivator%getFirstDeriv(sPrimeTmp, skOverCont, coords, species, iAt1, iAt2, orb)
        ...
        do iSpin = 1, nSpin
          sqrDMTmp(1:nOrb2,1:nOrb1) = &
              & reshape(DM(iOrig+1:iOrig+nOrb1*nOrb2,iSpin), (/nOrb2,nOrb1/))
          ! note factor of 2 for implicit summation over lower triangle of
          ! density matrix:
          shiftSprime(1:nOrb2,1:nOrb1) = ( &
              & matmul(sPrimeTmp(1:nOrb2,1:nOrb1,3), &
              & shift(1:nOrb1,1:nOrb1,iAt1,iSpin) ) &
              & + matmul(shift(1:nOrb2,1:nOrb2,iAt2f,iSpin), &
              & sPrimeTmp(1:nOrb2,1:nOrb1,3)) )
          intermed = intermed + &
              & sum(shiftSprime(1:nOrb2,1:nOrb1) * sqrDMTmp(1:nOrb2,1:nOrb1))
        end do
        vect(:) = coords(:,iAt1) - coords(:,iAt2)
        if (iAt1 == iAt2f) then
          prefac = 0.5_dp
        ...
        do iSpin = 1, nSpin
          ...
          shiftSprime(1:nOrb2,1:nOrb1) =  ( &
              & matmul(sPrimeTmp(1:nOrb2,1:nOrb1,1), &
              & shift(1:nOrb1,1:nOrb1,iAt1,iSpin) ) &
              & + matmul(shift(1:nOrb2,1:nOrb2,iAt2f,iSpin), &
              & sPrimeTmp(1:nOrb2,1:nOrb1,1)) )
          deriv(2) = deriv(2) + prefac * ( &
              & sum(shiftSprime(1:nOrb2,1:nOrb1) * sqrDMTmp(1:nOrb2,1:nOrb1) ))
        end do
      end do
    end do

  end subroutine getHelicalBlockLatForce
