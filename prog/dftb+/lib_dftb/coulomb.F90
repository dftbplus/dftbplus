...
#:endif
  ...
  public :: getMaxREwald, invRStress, invRHelix
  ...
  interface invRhelix
     module procedure helicalEwaldPotential
  end interface
...
99020 format ('Failure in getMaxREwald.', ' Error nr: ',I3)
  ...
  !> Main driving function for helical Ewald sums
  function helicalEwaldPotential(rq, r0, bc, eta, zeta_max, l_max, k_max, intervals) result(res)

    !> cylindrical coordinates of charges (r, z, theta)
    real(dp), intent(in) :: rq(3)

    !> point to evaluate potential (r, z, theta)
    real(dp), intent(in) :: r0(3)

    !> helical bounday condtions (z,theta)
    real(dp), intent(in) :: bc(:)

    !> Numerical Ewald split parameter.
    real(dp), intent(in) :: eta

    !> Maximum summation index in V^S.
    integer, intent(in) :: zeta_max

    !> Maximum l summation index in V^L.
    integer, intent(in) :: l_max

    !> Maximum k summation index in V^L (usually zero).
    integer, intent(in) :: k_max

    !> Number of numerical integration intervals in V^L.
    integer, intent(in) :: intervals

    real(dp) :: res, VS, VL, rp(3), r(3), r0cc(3), rpTmp(3)
    integer :: ii

    r0cc = r0 ! still need to fold to central cell
    r = (/r0cc(1),bc(1),bc(2)/)
    rp = (/rq(1),rq(2)-r0cc(2),rq(3)-r0cc(3)/)
    if (size(bc)==2) then ! only the pure helix operations present
      VS = helicalRspace(r(1), r(2), r(3), rp(1), rp(2), rp(3), eta, zeta_max)
      VL = helicalGspace(r(1), r(2), r(3), rp(1), rp(2), rp(3), eta, l_max, k_max, intervals)
      res =  VS + VL
    elseif (size(bc)==3) then ! helix plus C_n rotations, use multiple helices
      res = 0.0_dp
      do ii = 1, nint(bc(3))
        rpTmp = rp
        rpTmp(3) = mod(rp(3)-real(ii-1)*2.0_dp*pi/real(bc(3)),2.0_dp*pi)
        VS = helicalRspace(r(1), r(2), r(3), rpTmp(1), rpTmp(2), rpTmp(3), eta, zeta_max)
        VL = helicalGspace(r(1), r(2), r(3), rpTmp(1), rpTmp(2), rpTmp(3), eta, l_max, k_max,&
            & intervals)
        res = res + VS + VL
      end do
    end if
  ...
  function helicalRspace(r, T, theta, r_prime, T_prime, theta_prime, eta, zeta_max)

    !> Radius of helix of charges, starting positon (r, 0, 0).
    real(dp), intent(in) :: r

    !> Translational operation.
    real(dp), intent(in) :: T

    !> Angle of rotation operation.
    real(dp), intent(in) :: theta

    !> 1st cylindrical coordinate (r) of point at which the potential is evaluated.
    real(dp), intent(in) :: r_prime

    !> 2nd cylindrical coordinate (z) of point at which the potential is evaluated.
    real(dp), intent(in) :: T_prime

    !> 3rd cylindrical coordinate (theta) of point at which the potential is evaluated.
    real(dp), intent(in) :: theta_prime
    ...
    !> Resulting real space contribution
    real(dp) :: helicalRspace
    ...
    helicalRspace = 0.0_dp
    sqrt_eta = sqrt(eta)

    ! negative zeta
    do zeta = -zeta_max, -1
      R_zeta_2 = (zeta * T - T_prime)**2 + (r_prime**2 + r**2)&
          & - 2.0_dp * r_prime * r * cos(zeta * theta - theta_prime)
      R_zeta = sqrt(R_zeta_2)
      helicalRspace = helicalRspace + 1.0_dp / R_zeta - dgamit(0.5_dp, eta * R_zeta_2) * sqrt_eta
    end do

    ! zeta = 0, but only if we are not at a lattice point
    if ( any( abs( (/r_prime - r,theta_prime,T_prime/) ) > epsilon(1.0_dp))) then
      R_0_2 = T_prime**2 + (r_prime**2 + r**2) - 2.0_dp * r_prime * r * cos(theta_prime)
      R_0 = sqrt(R_0_2)
      helicalRspace = helicalRspace + 1.0_dp / R_0 - dgamit(0.5_dp, eta * R_0_2) * sqrt_eta
    end if
    ...
    ! add the background charge term if this was a charged line - note this is a line charge at the
    ! axis
    ! helicalRspace = helicalRspace - dgamic(0.0_dp, eta * r_prime**2) / T

  end function helicalRspace


  !> Calculates long-ranged V^L component of helical Ewald summation.
  function helicalGspace(r, T, theta, r_prime, T_prime, theta_prime, eta, l_max, k_max, intervals)
    ...
    real(dp) :: helicalGspace

    real(dp) :: x ! (t in helical Ewald paper)
    ...
    four_T_sq_rec = 1.0_dp / (4.0_dp * T**2)
    two_rprime_r = 2.0_dp * r_prime * r
    r_sq_sum = r_prime_sq + r_sq

    step = eta / intervals
    helicalGspace = 0.0_dp

    ! pre-fill arrays for numberical integration
    do i = 1, intervals

       ! start at 1, not zero, because at x=0 (t in paper) we put in
       !  the limit manually

       ! divide by x (t in paper) at the end of each numerical
       ! integration step divide by T at the very end of numerical
       ! integration

       ! zbesi returns an array of values in steps of 1 for the order,
       ! so we pre -fill it as well as the array for variable of
       ! integration, x (t in paper), here and initialize integrand

       x = step * i
       integrand = 0.0_dp
       call zbesi(two_rprime_r*x, zi, fnu, 1, l_max+1, bessel_array, bessel_dummy_complex_array,&
           & nz, ierr)

       if ((nz /= 0) .or. (ierr /= 0)) then
          call error('Error or underflow in bessel subroutine')
       end if

       ! add 1:l_max arrays first, to double later
       do l = 1, l_max
         do k = -k_max, k_max
           integrand = integrand + cos(-l*theta_prime+(l*theta+2.0_dp*pi*k)*T_prime/T)&
               & * bessel_array(l+1) * exp(-r_sq_sum*x-four_T_sq_rec*(2.0_dp*pi*k+theta*l)**2/x)
         end do
       end do

       integrand = integrand * 2.0_dp
       ...
       do k = -k_max, k_max
         ...
         integrand = integrand + cos(2.0_dp*pi*k*T_prime/T) * bessel_array(1)&
             & * exp(-r_sq_sum*x-four_T_sq_rec*(2.0_dp*pi*k)**2/x)
       end do

       ! now l=0 k=0
       integrand = integrand +  bessel_array(1) * exp(-r_sq_sum*x) - exp(-r_prime_sq*x)
       integrand = integrand / x

       ! add up for numerical integration
       if (i == intervals) then
         helicalGspace = helicalGspace + 0.5_dp * integrand
       else
         helicalGspace = helicalGspace + integrand
       end if

    end do

    ! this is 1/2 * the limit of the helicalGspace integrand as it approaches 0
    helicalGspace = (helicalGspace - 0.5_dp * r_sq) / T
    helicalGspace = helicalGspace * step

    if (abs(r_prime - r) < epsilon(1.0_dp) .and. abs(theta_prime) < epsilon(1.0_dp) &
        & .and. abs(T_prime) < epsilon(1.0_dp)) then
      ! self term for charge
      helicalGspace = helicalGspace - 2.0_dp * sqrt(eta / pi)
    end if

  end function helicalGspace
