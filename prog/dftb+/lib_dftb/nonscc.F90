...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine getFirstDerivFiniteDiff(deriv, skCont, coords, species, atomI,&
    ...
    real(dp) :: interSk(getMIntegrals(skCont))    ! interpolated H integs.
    real(dp) :: vect(3), dist
    integer  :: ii, jj
    integer :: sp1, sp2
  ...
  subroutine getFirstDerivRichardson(deriv, skCont, coords, species, atomI,&
    ...
    do iCart = 1, 3
      ...
      ! initial finite difference, chosen as an exactly representable value in
      ! floating point
      hh = 0.125_dp
      dd(:,:,:,:) = 0.0_dp
      ...
      ! want value of ii to be consistent however the loop exits, so
      ! using a while instead of for loop
      do while (ii < maxrows .and. .not. all(tConverged))
        ...
        if (hh <= epsilon(1.0_dp)**0.25_dp) then
          ! rounding error on order of epsilon/h^2 and central difference
          ! formula truncation error of O(h^2), so want to keep above
          ! epsilon/h^4. If the function gets large, then max(1,|f|)
          ! epsilon^(1/4) might be better
          exit
        end if
        ii = ii + 1
        do kk = 1, 2
        ...
        do ll = 1, size(dd, dim=2)
          do kk = 1, size(dd, dim=1)
            ...
            if (diff <= tol * dd(kk,ll,mm,ii)) then
              ! copy converged value over all entries of the table for
              ! this element
  ...
  subroutine getSecondDerivFiniteDiff(deriv, skCont, coords, species, atomI,&
    ...
    do ii = 1, 3
      do jj = 1, 3
        ...
        if (ii == jj) then ! treat diagonal case separately
          cycle
        end if
