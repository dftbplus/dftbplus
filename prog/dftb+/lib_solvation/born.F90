...
contains
  ...
  subroutine initialize(self, input, nAtom, species0, speciesNames, latVecs)
    ...
    if (input%tCM5) then
      allocate(self%cm5)
      if (self%tPeriodic) then
        call self%cm5%initialize(nAtom, species0, speciesNames, input%rCutoff, latVecs)
      else
        call self%cm5%initialize(nAtom, species0, speciesNames, input%rCutoff)
      end if
    end if
  ...
  subroutine addGradients(self, neighList, species, coords, img2CentCell, gradients)
    ...
    real(dp) :: iAt1
    real(dp) :: sigma(3, 3)
    real(dp), allocatable :: dEdcm5(:), unity(:)
    ...
    if (allocated(self%cm5)) then
      allocate(dEdcm5(self%nAtom), unity(self%nAtom), derivs(3, self%nAtom))
      dEdcm5(:) = 0.0_dp
      derivs(:, :) = 0.0_dp
      ...
      unity(:) = 1.0_dp
      call self%cm5%addGradients(unity, derivs)
      do iAt1 = 1, self%nAtom
        gradients(:, iAt1) = gradients(:, iAt1) - dEdcm5(iAt1) * derivs(:, iAt1)
      end do
    end if
