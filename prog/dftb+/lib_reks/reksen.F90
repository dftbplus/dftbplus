...
module dftbp_reksen
  ...
  subroutine activeOrbSwap(eigenvecs, self)
  ...
  subroutine getFilling(filling, self)
    ...
    if (self%tSSR22) then
      call getFilling22_(filling, self)
    else if (self%tSSR44) then
      call error("SSR(4,4) is not implemented yet")
    end if
  ...
  subroutine calcSaReksEnergy(energy, self)
  ...
  subroutine solveSecularEqn(env, denseDesc, neighbourList, &
    ...
    call printReksSSRInfo(Wab, tmpEn, StateCoup, self)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine MOswap22_(eigenvecs, self)
    ...
    if (self%Efunction == 1) then
      ...
      ! SA-REKS charge
      fa = (n_a*self%SAweight(1) + self%SAweight(2)) * 0.5_dp
      fb = (n_b*self%SAweight(1) + self%SAweight(2)) * 0.5_dp
    end if

    if (fa < fb) then
      write(stdOut,'(A6,F9.6,A20,I4,A8,I4,A8)') " fa = ", fa, &
          & ", MO swap between a(", self%Nc+1, ") and b(", self%Nc+2, ") occurs"
      tmpMO(:) = eigenvecs(:,self%Nc+1)
      eigenvecs(:,self%Nc+1) = eigenvecs(:,self%Nc+2)
      eigenvecs(:,self%Nc+2) = tmpMO
    end if
  ...
  subroutine getFilling22_(filling, self)
    ...
    if (self%Efunction == 1) then
      ! REKS charge
      filling(self%Nc+1) = n_a
      filling(self%Nc+2) = n_b
    else if (self%Efunction == 2) then
      ! SA-REKS charge
      filling(self%Nc+1) = n_a*self%SAweight(1) + self%SAweight(2)
      filling(self%Nc+2) = n_b*self%SAweight(1) + self%SAweight(2)
    end if
