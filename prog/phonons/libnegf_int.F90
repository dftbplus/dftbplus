...
#:endif
  use dftbp_extlibs_negf, only : getel, lnParams, pass_DM, Tnegf
  use dftbp_extlibs_negf, only : kb, units, convertHeatCurrent, convertHeatConductance
  use dftbp_extlibs_negf, only : z_CSR, z_DNS, READ_SGF, COMP_SGF, COMPSAVE_SGF
  use dftbp_extlibs_negf, only : DELTA_SQ, DELTA_W, DELTA_MINGO 
  use dftbp_extlibs_negf, only : associate_lead_currents, associate_ldos, associate_transmission
  use dftbp_extlibs_negf, only : compute_phonon_current, thermal_conductance 
  ...
  use dftbp_extlibs_negf, only : init_negf, init_structure, pass_hs, set_bp_dephasing, set_scratch
  use dftbp_extlibs_negf, only : set_drop, set_elph_block_dephasing, set_elph_dephasing
  use dftbp_extlibs_negf, only : set_elph_s_dephasing, set_ldos_indexes, set_tun_indexes, set_params
  use dftbp_extlibs_negf, only : writememinfo, writepeakinfo, dns2csr, csr2dns, nzdrop, printcsr
