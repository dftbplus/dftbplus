...
# include "assert.h"  
  ...
  ALLOCATE_(buffer, (nPoints(1), nPoints(2), nPoints(3)))
  if (tCalcTotChrg) then
    ALLOCATE_(totChrg, (nPoints(1), nPoints(2), nPoints(3)))
    totChrg = 0.0_dp
    if (tPlotTotSpin) then
      ALLOCATE_(spinUp, (nPoints(1), nPoints(2), nPoints(3)))
    end if
  end if
  ...
  if (nBox > 1) then
    !! If tFillBox is off, coordinates must be repeated here. Otherwise
    !! the part for filling with atoms will do that.
    if (.not. tFillBox) then
      ALLOCATE_(coords, (3, size(geo%coords)))
  ...
  if (tCalcAtomDens) then
    ALLOCATE_(atomicChrg, (nPoints(1), nPoints(2), nPoints(3), 1))
    ALLOCATE_(orbitalOcc, (nOrb, 1))
    ind = 1
    do iAtom = 1, geo%nAtom
...
9989  format('Calc-Id:',I11,', atomdens')
  ...
  if (tFillBox) then
    !! Shifting plotted region by integer lattice vectors, to have its center
    !! as close to the center of the lattice unit cell as possible.
    ...
    !! We need all cells around, which could contain atoms in the sphere,
    !! drawn from the center of the unit cell, containing entire plotted region
    mDist = 0.0_dp
    do i1 = 0, 1
      do i2 = 0, 1
        do i3 = 0, 1
...
contains
  ...
  !! @param geo           Geometry information about the structure
  !! @param atomicNumbers Atomic numbers correpsonding to each species
  !! @param gridVecs      Grid vectors
  !! @param origin        Origin of the grid
  !! @param gridVal       Value of the 3D function in the grid points
  !! @param fileName      Name of the file to create
  !! @param comments      First two comment lines of the file
  !! @param repeatBox     How often the grid should be repeated along the
  !!                      direction of the grid vectors
