...
c
         do i=ist,ifn
         q(i,2)=(5.d0*q(i,1)+15.d0*q(i,3)-5.d0*q(i,5)+q(i,7))*.0625d0
...
c
     ...
     +      resf(i+1,j+1,k-1)+2.d0*(resf(i-1,j,k-1)+resf(i+1,j,k-1)+
     +      resf(i,j-1,k-1)+resf(i,j+1,k-1))+4.d0*resf(i,j,k-1))*.0625d0
...
c
     ...
     +      resf(i,jm1,k-1)+resf(i,jp1,k-1))+4.d0*resf(i,j,k-1))*.0625d0
...
c     x=xa,xb; y=yc,yd; z=ze,zf cornors
     ...
     +    resf(i,jm1,km1)+resf(i,jp1,km1))+4.d0*resf(i,j,km1))*.0625d0
        rk=(resf(im1,jm1,k)+resf(ip1,jm1,k)+resf(im1,jp1,k)+
     +    resf(ip1,jp1,k)+2.d0*(resf(im1,j,k)+resf(ip1,j,k)+
     +    resf(i,jm1,k)+resf(i,jp1,k))+4.d0*resf(i,j,k))*.0625d0
        rp=(resf(im1,jm1,kp1)+resf(ip1,jm1,kp1)+resf(im1,jp1,kp1)+
     +    resf(ip1,jp1,kp1)+2.d0*(resf(im1,j,kp1)+resf(ip1,j,kp1)+
     +    resf(i,jm1,kp1)+resf(i,jp1,kp1))+4.d0*resf(i,j,kp1))*.0625d0
        rhsc(ic,jc,kc) = 0.25d0*(rm+2.d0*rk+rp)
        end do
      end do
c
...
c
     ...
     +      resf(i,j-1,km1)+resf(i,j+1,km1))+4.d0*resf(i,j,km1))*.0625d0
          rk=(resf(im1,j-1,k)+resf(ip1,j-1,k)+resf(im1,j+1,k)+
     +      resf(ip1,j+1,k)+2.d0*(resf(im1,j,k)+resf(ip1,j,k)+
     +      resf(i,j-1,k)+resf(i,j+1,k))+4.d0*resf(i,j,k))*.0625d0
          rp=(resf(im1,j-1,kp1)+resf(ip1,j-1,kp1)+resf(im1,j+1,kp1)+
     +      resf(ip1,j+1,kp1)+2.d0*(resf(im1,j,kp1)+resf(ip1,j,kp1)+
     +      resf(i,j-1,kp1)+resf(i,j+1,kp1))+4.d0*resf(i,j,kp1))*.0625d0
          rhsc(ic,jc,kc) = 0.25d0*(rm+2.d0*rk+rp)
        end do
      end do
      end do
...
c     y=yc,x interior
     ...
     +       +u(i-2,3)-2.d0*u(i-1,3)+2.d0*u(i+1,3)-u(i+2,3)) / dxxxy4
      uxy3=(5.d0*u(i-1,1)-5.d0*u(i+1,1)-18.d0*u(i-1,2)+18.d0*u(i+1,2)
     +   +24.d0*u(i-1,3)-24.d0*u(i+1,3)-14.d0*u(i-1,4)+14.d0*u(i+1,4)
     +   +3.d0*u(i-1,5)-3.d0*u(i+1,5))
     + /dxyyy4
      else if (j.eq.2) then
c     y=yc+dly,x interior
      ux3y=(u(i-2,1)-2.d0*u(i-1,1)+2.d0*u(i+1,1)-u(i+2,1)
     +      -u(i-2,3)+2.d0*u(i-1,3)-2.d0*u(i+1,3)+u(i+2,3))
     + /dxxxy4
      uxy3=(u(i-1,1)-u(i+1,1)-2.d0*u(i-1,2)+2.d0*u(i+1,2)
     +      +2.d0*u(i-1,4)-2.d0*u(i+1,4)-u(i-1,5)+u(i+1,5))
     + /dxyyy4
      else if (j.eq.ny-1) then
c     y=yd-dly, x interior
      ux3y=(u(i-2,n2)-2.d0*u(i-1,n2)+2.d0*u(i+1,n2)-u(i+2,n2)
     +      -u(i-2,ny)+2.d0*u(i-1,ny)-2.d0*u(i+1,ny)+u(i+2,ny))
     + /dxxxy4
...
c     at y=yd, x interior
      ux3y=(-u(i-2,n2)+2.d0*u(i-1,n2)-2.d0*u(i+1,n2)+u(i+2,n2)
     + +4.d0*u(i-2,n1)-8.d0*u(i-1,n1)+8.d0*u(i+1,n1)-4.d0*u(i+2,n1)
     + -3.d0*u(i-2,ny)+6.d0*u(i-1,ny)-6.d0*u(i+1,ny)+3.d0*u(i+2,ny))
     + /dxxxy4
      uxy3=(-3.d0*u(i-1,n4)+3.d0*u(i+1,n4)+14.d0*u(i-1,n3)-
     +   14.d0*u(i+1,n3)-24.d0*u(i-1,n2) +24.d0*u(i+1,n2)+
     +   18.d0*u(i-1,n1)-18.d0*u(i+1,n1)-5.d0*u(i-1,ny)+5.d0*u(i+1,ny))
     +   /dxyyy4
      end if
...
c     x=xb-dlx,y interior
      ux3y=(-u(m4,j-1)+6.d0*u(m3,j-1)-12.d0*u(m2,j-1)+10.d0*u(m1,j-1)-
     +     3.d0*u(nx,j-1)+u(m4,j+1)-6.d0*u(m3,j+1)+12.d0*u(m2,j+1)-
     +    10.d0*u(m1,j+1)+3.d0*u(nx,j+1)) /dxxxy4
...
c     at x=xb-dlx, y=yc
     ...
     + +3.d0*u(m2,5)-3.d0*u(nx,5))
     + /dxyyy4
      else if (j.eq.2) then
c     x=xb-dlx,y=yc+dly
      ux3y=(-u(m4,1)+6.d0*u(m3,1)-12.d0*u(m2,1)+10.d0*u(m1,1)-
     +  3.d0*u(nx,1)+u(m4,3)-6.d0*u(m3,3)+12.d0*u(m2,3)-10.d0*u(m1,3)+
     +  3.d0*u(nx,3))/dxxxy4
...
c     at x=xb-dlx,y=yd-dly
     ...
     + -12.d0*u(m2,n2)+12.d0*u(nx,n2)+10.d0*u(m2,n1)-10.d0*u(nx,n1)
     + -3.d0*u(m2,ny)+3.d0*u(nx,ny)) / dxyyy4
      else if (j.eq.ny) then
c     at x=xb.dlx,y=yd
      ux3y=(u(m4,n2)-6.d0*u(m3,n2)+12.d0*u(m2,n2)-10.d0*u(m1,n2)+
     + 3.d0*u(nx,n2)-4.d0*u(m4,n1)+24.d0*u(m3,n1)-48.d0*u(m2,n1)+
     + 40.d0*u(m1,n1)-12.d0*u(nx,n1)+3.d0*u(m4,ny)-18.d0*u(m3,ny)+
     + 36.d0*u(m2,ny)-30.d0*u(m1,ny)+9.d0*u(nx,ny))/ dxxxy4
      uxy3=(-3.d0*u(m2,n4)+3.d0*u(nx,n4)+14.d0*u(m2,n3)-14.d0*u(nx,n3)
     + -24.d0*u(m2,n2)+24.d0*u(nx,n2)+18.d0*u(m2,n1)-18.d0*u(nx,n1)
...
c     x=xb,y interior
     ...
     +18.d0*u(m1,j-1)-5.d0*u(nx,j-1)+3.d0*u(m4,j+1)-14.d0*u(m3,j+1)+
     +24.d0*u(m2,j+1)-18.d0*u(m1,j+1)+5.d0*u(nx,j+1)) / dxxxy4
...
c     x=xb,y=yd
      ux3y=(3.d0*u(m4,n2)-14.d0*u(m3,n2)+24.d0*u(m2,n2)-18.d0*u(m1,n2)+
     +5.d0*u(nx,n2)-12.d0*u(m4,n1)+56.d0*u(m3,n1)-96.d0*u(m2,n1)+
     +72.d0*u(m1,n1)-20.d0*u(nx,n1)+9.d0*u(m4,ny)-42.d0*u(m3,ny)+
     ...
     + -4.d0*u(m3,n2)+16.d0*u(m2,n2)-20.d0*u(m1,n2)+8.d0*u(nx,n2)
     + +5.d0*u(m3,n1)-20.d0*u(m2,n1)+25.d0*u(m1,n1)-10.d0*u(nx,n1)
     + -2.d0*u(m3,ny)+8.d0*u(m2,ny)-10.d0*u(m1,ny)+4.d0*u(nx,ny))
     + / dxxyy
      end if
