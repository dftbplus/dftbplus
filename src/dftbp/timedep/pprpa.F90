...
contains
  ...
  @:ASSERT(allocated(RPA))
    ...
    do isym = 1, size(symmetries)
      ...
      call buildAndDiagppRPAmatrix(env%stdOut, RPA%tTDa, sym, grndEigVal(:,1), nocc, nvir, nxvv, nxoo,&
  ...
  subroutine buildAndDiagppRPAmatrix(output, tTDA, sym, eigVal, nocc, nvir, nxvv, nxoo, ind, gamma_eri,&
