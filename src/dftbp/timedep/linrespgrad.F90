...
contains
  ...
  subroutine LinRespGrad_old(env, this, denseDesc, grndEigVecs, grndEigVal, sccCalc, dq, coord0, SSqr,&
  ...
  #:endif   
    ...
    if (.not. tZVector) then
      ...
      do iLev = nStartLev, nEndLev
        ...
        call getZVectorEqRHS(tRangeSep, xpy(:,iLev), xmy(:,iLev), win, env, denseDesc,&
        ...
        do nCoupLev = this%indNACouplings(1), this%indNACouplings(2)-1
          do mCoupLev = nCoupLev+1, this%indNACouplings(2)
            ...
            if(nCoupLev == 0) then
              ...
              call grndToExcDensityMatrices(tRangeSep, xpym, xmym, win, env, denseDesc, nocc_ud,& 
  ...
  subroutine buildAndDiagExcMatrixStratmann(tSpin, subSpaceFactor, wij, sym, win, nocc_ud, nvir_ud,&
      ...
      & spinW, transChrg, eval, xpy, xmy, onsMEs, orb, tRangeSep, lrGamma, tZVector)
  ...
  #:if WITH_SCALAPACK
          ...
          call actionAplusB_MPI(locSize, vOffset, tSpin, wij, sym, win, nocc_ud, nvir_ud, &
            & nxoo_ud, nxvv_ud, nxov_ud, nxov_rd, iaTrans, getIA, getIJ, getAB, env, denseDesc,&
            & ovrXev, ovrXevGlb, grndEigVecs, eigVecGlb, filling, sqrOccIA, gammaMat, species0,&
            & spinW, onsMEs, orb, .true., transChrg, vecB(:,ii), vP(:,ii), tRangeSep, lrGamma)
          call actionAminusB_MPI(locSize, vOffset, tSpin, wij, win, nocc_ud, nvir_ud, nxoo_ud,&
            & nxvv_ud, nxov_ud, nxov_rd, iaTrans, getIA, getIJ, getAB, env, denseDesc, ovrXev,&
            & grndEigVecs, filling, sqrOccIA, transChrg, vecB(:,ii), vM(:,ii), tRangeSep,& 
            & lrGamma)
  ...
  !> do not depend on Z.
   subroutine getZVectorEqRHS(tRangeSep, xpy, xmy, win, env, denseDesc, homo, transChrg, getIA, getIJ,&
  ...
  subroutine grndToExcDensityMatrices(tRangeSep, xpy, xmy, win, env, denseDesc, homo, transChrg,& 
  ...
  subroutine getNadiaZvectorEqRHS(tRangeSep, xpyn, xmyn, xpym, xmym, win, env, denseDesc, homo,& 
