...
contains
  ...
  subroutine writeRestartFile(rho, rhoOld, coord, veloc, time, dt, fileName, isAsciiFile, error)
    ...
    if (iErr /= 0) then
      ...
      @:RAISE_ERROR(error, -1, error_string)
    end if
  ...
  subroutine readRestartFile(rho, rhoOld, coord, veloc, time, dt, fileName, isAsciiFile, error)
    ...
    if (isAsciiFile) then
      ...
      if (version /= iDumpFormat) then
        @:RAISE_ERROR(error, -1, "Unknown TD restart format")
      end if
