...
contains
  ...
  subroutine writeRestartFile(exc, rho, rhoOld, coord, veloc, time, dt, fileName, isAsciiFile)
    ...
    if (iErr /= 0) then
      ...
      @:RAISE_EXCEPTION(exc, -1, error_string)
    end if
  ...
  subroutine readRestartFile(exc, rho, rhoOld, coord, veloc, time, dt, fileName, isAsciiFile)
    ...
    if (isAsciiFile) then
      ...
      if (version /= iDumpFormat) then
        @:RAISE_EXCEPTION(exc, -1, "Unknown TD restart format")
      end if
