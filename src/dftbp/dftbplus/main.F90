  ...
  #:endif
    ...
    if (allocated(this%electronDynamics)) then
      call runDynamics(this%electronDynamics, this%boundaryCond, this%eigvecsReal, this%H0,&
          ...
          & this%nNeighbourSK, this%nNeighbourCam, this%denseDesc%iAtomStart, this%iSparseStart,&
          ...
          & this%hybridXc, this%qDepExtPot, this%dftbU, this%iAtInCentralRegion, this%tFixEf,&
  ...
  #:endif
    ...
    if (this%isLinResp) then
      call calculateLinRespExcitations(env, this%linearResponse, this%parallelKS, this%scc,&
          ...
          & this%densityMatrix%deltaRhoOut, this%excitedDerivs, this%occNatural,&
          & this%hybridXc)
    end if
...
#:endif
  ...
  #:if WITH_SCALAPACK
    real(dp) :: localDeltaRhoInSqr(orb%nOrb, orb%nOrb, size(qOutput, dim=3))
    real(dp) :: localDeltaRhoOutSqr(orb%nOrb, orb%nOrb, size(qOutput, dim=3))
  #:endif
    ...
    if ((.not. tConverged) .and. (iSCCiter /= maxSccIter .and. .not. tStopScc)) then
      if ((iSCCIter + iGeoStep) == 1 .and. (nSpin > 1 .and. .not. tReadChrg)) then
        ...
        #:if WITH_SCALAPACK
          ...
          localDeltaRhoInSqr(:,:,:) = 0.0_dp
          call getFullFromDistributed(env, denseDesc, orb, parallelKS, species0,&
              & densityMatrix%deltaRhoIn, localDeltaRhoInSqr)

          localDeltaRhoOutSqr(:,:,:) = 0.0_dp
          call getFullFromDistributed(env, denseDesc, orb, parallelKS, species0,&
              & densityMatrix%deltaRhoOut, localDeltaRhoOutSqr)

          ! Re-allocate difference, this time for full density collected from all MPI ranks
          deltaRhoDiffSqr = localDeltaRhoOutSqr - localDeltaRhoInSqr

          if (env%tGlobalLead) then
            call mix(pChrgMixer, localDeltaRhoInSqr, deltaRhoDiffSqr)
          end if
          call mpifx_bcast(env%mpi%globalComm, localDeltaRhoInSqr)
          ! scatter mixed full, square density matrix to MPI ranks
          call scatterFullToDistributed(env, denseDesc, parallelKS, species0, localDeltaRhoInSqr,&
  ...
  subroutine calculateLinRespExcitations(env, linearResponse, parallelKS, sccCalc, qOutput, q0,&
    ...
    if (tLinRespZVect) then
      ...
      call LinResp_addGradients(tSpin, linearResponse, denseDesc%iAtomStart, eigvecsReal, eigen,&
          ...
          & dftbEnergy%Eexcited, energies, excitedDerivs, nonSccDeriv, rhoSqrReal, deltaRhoOut,&
