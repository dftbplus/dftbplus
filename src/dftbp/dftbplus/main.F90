...
module dftbp_dftbplus_main
  ...
  use dftbp_math_symmetrize, only : TEquivalentAtoms
    ...
    if (this%tSccCalc) then
      ...
      if (.not. this%isHybridXc) then
        call getNextInputCharges(env, this%pChrgMixerReal, this%qOutput, this%qOutRed, this%orb,&
            ...
            & this%qInput, this%qInpRed, this%equivalentAtoms, sccErrorQ, tConverged, this%dftbU,&
...
#:endif
  ...
  subroutine getNextInputCharges(env, pChrgMixerReal, qOutput, qOutRed, orb, nIneqOrb, iEqOrbitals,&
      ...
      & qInput, qInpRed, equivalentAtoms, sccErrorQ, tConverged, dftbU, qBlockOut, iEqBlockDftbU,&
    ...
    !> Mapping between symmetry equivalent atoms
    type(TEquivalentAtoms), intent(in), allocatable :: equivalentAtoms
    ...
    if (allocated(equivalentAtoms)) then
      ! symmetrize charges between equivalent atoms
      call equivalentAtoms%mapAtomicQuantities(qInput)
      if (allocated(qBlockIn)) call equivalentAtoms%mapAtomicQuantities(qBlockIn)
      if (allocated(qiBlockIn)) call equivalentAtoms%mapAtomicQuantities(qiBlockIn)
      if (nIneqDip > 0) call equivalentAtoms%mapAtomicQuantities(multipoleInp%dipoleAtom)
      if (nIneqQuad > 0) call equivalentAtoms%mapAtomicQuantities(multipoleInp%quadrupoleAtom)
    end if
