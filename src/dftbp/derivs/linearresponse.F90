...
#:if WITH_SCALAPACK
  use dftbp_dftb_sparse2dense, only : packRhoCplxBlacs, packRhoHelicalCplxBlacs,&
      ...
      & unpackHSdk
  use dftbp_extlibs_scalapackfx, only : CSRC_, DLEN_, MB_, NB_, RSRC_, blocklist, pblasfx_pgemm,&
      & pblasfx_phemm, pblasfx_psymm, pblasfx_ptran, pblasfx_ptranc, scalafx_getdescriptor,&
      & scalafx_indxl2g
  use dftbp_extlibs_mpifx, only : MPI_SUM, mpifx_allreduceip
#:else
  use dftbp_dftb_sparse2dense, only : iPackHS, packHelicalHS, packHS, unpackHelicalHS,&
      & unpackHPauli, unpackHS, unpackHSdk
...
#:endif
  ...
  subroutine dPsidK(env, parallelKS, eigvals, eigVecsCplx, ham, over, orb, nAtom, species,&
      ...
      & taggedResultsFile, tWriteDetailedOut, fdDetailedOut, errStatus)
  ...
  #:endif
    ...
    if (.not.allocated(eigVecsCplx)) then
      @:RAISE_ERROR(errStatus, -1, "Missing complex eigenvectors for perturbation")
    end if
  ...
  #:if WITH_SCALAPACK
    ...
    call TRotateDegen_init(transform)
    ...
    do iKS = 1, parallelKS%nLocalKS
      ...
      do iCart = 1, 3
        ...
        call transform%generateUnitary(env, work2local, eigvals(:,iK,iS),&
            & eigVecsTransformed, denseDesc, isTransformed, errStatus)
        ...
        do jj = 1, size(workLocal,dim=2)
          ...
          do ii = 1, size(workLocal,dim=1)
            ...
            if (abs(eigvals(jGlob,iK,iS) - eigvals(iGlob,iK,iS)) < epsilon(0.0_dp)) then
              ! degenerate, so no contribution, either due to being same state or by
              ! orthogonalisation against perturbation
              work2Local(ii,jj) = (0.0_dp,0.0_dp)
            else
              ! zero temperature form is safe
