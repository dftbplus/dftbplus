...
module dftbp_derivs_perturb
  ...
  use dftbp_common_constants, only : AA__Bohr, Hartree__eV, quaternionName
...
contains
  ...
  subroutine dPsidx(this, env, parallelKS, filling, eigvals, eigVecsReal, eigVecsCplx, rhoPrim,&
...
!                  & dEi, dPsiCmplx, iAt)
            ...
            @:RAISE_ERROR(errStatus, -1, "Shouldn't be here in dPsidx routine")
    ...
    do iS = 1, nSpin
      do iAt = 1, nAtom
        ...
        do iOrb = 1, size(dEi,dim=1)
          write(stdOut, "(3F16.8)") dEi(iOrb, 1, iS, :, iAt) * Hartree__eV * AA__Bohr
        end do
      end do
    end do
