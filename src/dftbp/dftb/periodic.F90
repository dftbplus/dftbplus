...
contains
  ...
  #:if WITH_MPI
    if (present(env)) then
      call distributeAtoms(env, env%mpi%nodeComm%rank, env%mpi%nodeComm%size, nAtom, startAtom, endAtom,&
  ...
  subroutine getNrOfNeighboursForAll(env, nNeighbourSK, neigh, cutoff)
    ...
    do iAtom = 1, nAtom
      nNeighbourSK(iAtom) = getNrOfNeighbours(env, neigh, cutoff, iAtom)
    end do
  ...
  function getNrOfNeighbours(env, neigh, cutoff, iAtom) result(nNeighbour)
...
99010 format ('Cutoff (', E16.6, ') greater than last cutoff ', '(', E13.6,&
      ...
      call warning(env%stdOut, strError)
    end if
  ...
  subroutine distributeAtoms(env, mpiRank, mpiSize, nAtom, startAtom, endAtom, error)
    ...
    if (nAtom < mpiSize) then
      call warning(env%stdOut, "Cannot parallelize atomic distance computation: &
