...
#:endif
  ...
  contains
    ...
    !> Sums up the potential gradients at specified coordinates due to charges at other positions
    procedure :: getPotentialGradient
...
contains
  ...
  !> Updates with changed charges for the instance.
  subroutine updateCharges(this, env, orb, species, deltaQAtom)
    ...
    !> Negative gross charge for each atom
...
#:endif
  ...
  subroutine addInvRPrimeClusterAsymm(env, nAtom0, nAtom1, coord0, coord1, charge0, charge1,&
    ...
    !> Compute the derivative of Hamiltonian (i.e. potential derivative)? Otherwise, compute the
    !! force
...
99020 format ('Failure in getMaxREwald.', ' Error nr: ',I3)
  ...
  !> Calculates the summed 1/R vector from charges at specified locations (like interaction of atoms
  !! with point charges).
  subroutine getPotential(this, env, coords, chargeCoords, charges, potential, blurWidths,&
    ...
    !> Coordinates of the charges
    real(dp), intent(in) :: chargeCoords(:,:)

    !> Charges
    real(dp), intent(in) :: charges(:)

    !> Potential at each specified point
  ...
  !> Calculates the summed -1/R**2 vector from charges at specified locations (like interaction of
  !! atoms with point charges).
  subroutine getPotentialGradient(this, env, coords, chargeCoords, charges, gradient, blurWidths)
    ...
    !> Potential gradient at each specified point
    ...
    if (this%boundaryCond_ == boundaryCondsEnum%cluster) then
      call addInvRPrimeClusterAsymm(env, size(coords, dim=2), size(chargeCoords, dim=2),&
          & coords, chargeCoords, placeHolderCharges, charges, gradient, placeHolderGradient,&
          & tHamDeriv=.true., blurWidths1=blurWidths)
    else
      call addInvRPrimePeriodicAsymm(env, size(coords, dim=2), size(chargeCoords, dim=2),&
          & coords, chargeCoords, placeHolderCharges, charges, this%rLatPoints_,&
          & this%gLatPoints_, this%alpha, this%volume_, gradient, placeHolderGradient,&
          & tHamDeriv=.true., blurWidths1=blurWidths)
    end if

  end subroutine getPotentialGradient
  ...
  subroutine addExternalPotGrad(this, env, atomCoords, extChargeCoords, atomCharges,&
    ...
    if (this%boundaryCond_ == boundaryCondsEnum%cluster) then
      call addInvRPrimeClusterAsymm(env, size(atomCoords,dim=2), size(extChargeCoords,dim=2),&
          & atomCoords, extChargeCoords, atomCharges, extCharges, atomGrads, extChargeGrads,&
          & tHamDeriv, extChargeBlurWidths)
    else
      call addInvRPrimePeriodicAsymm(env, size(atomCoords,dim=2), size(extChargeCoords,dim=2),&
          & atomCoords, extChargeCoords, atomCharges, extCharges, this%rLatPoints_,&
          & this%gLatPoints_, this%alpha, this%volume_, atomGrads, extChargeGrads, tHamDeriv,&
          & extChargeBlurWidths)
    end if
