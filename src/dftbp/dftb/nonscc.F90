...
contains
  ...
  !> Calculates the first derivative of H0 or S matrix with respect to iAt position
    ...
    case (diffTypes%finiteDiff)
      ...
      do iCart = 1, 3
        ...
        do ii = -1, 1, 2
          ...
          if (ii == 1) then
            ! onsites cancel in difference formula, so can use build S for either H0 or S
