...
/*!
  ...
  DFT+ equivalent of ASI_matrix_descr_t
*/
typedef struct
{
  int matrix_type;   //!< Type of a matrix. See DFTBP_MATRIX_TYPE_* constants. For example \ref DFTBP_MATRIX_TYPE_HERMSYM
  int storage_type;  //!< Type of a matrix storage. See ASI_STORAGE_TYPE_* constants. For example \ref DFTBP_STORAGE_TYPE_TRIL
} DftbPlusMatrixDescr;

/*! Generic matrix type for \ref DftbPlusMatrixDescr::matrix_type  */
#define DFTBP_MATRIX_TYPE_GENERIC 0
/*! Hermitian or symmetric matrix type for \ref DftbPlusMatrixDescr::matrix_type  */
...
/*! Only lower triangle stored in column-major order (upper for row-major). For \ref DftbPlusMatrixDescr::storage_type  */
#define DFTBP_STORAGE_TYPE_TRIL  1
/*! Only upper triangle stored in column-major order (lower for row-major). For \ref DftbPlusMatrixDescr::storage_type  */
 ...
 */
typedef void (*DMHSCallBackFunc)(void *aux_ptr, int iK, int iS, int *blacs_descr, const void *blacs_data, DftbPlusMatrixDescr *matrix_descr);
 ...
 */
typedef int (*SetDMHSCallBackFunc)(void *aux_ptr, int iK, int iS, int *blacs_descr, void *blacs_data, DftbPlusMatrixDescr *matrix_descr);
