...
9989    format('Calc-Id:',I11,', atomdens')
        ...
        call writeCubeFile(wp%input%geo, wp%aNr%atomicNumbers, wp%loc%gridVec, wp%opt%gridOrigin,&
            & buffer, fileName, comments=comments, repeatBox=wp%opt%repeatBox)
...
9993 format('Calc-Id:',I11,', abs2')
    ...
    call writeCubeFile(wp%input%geo, wp%aNr%atomicNumbers, wp%loc%gridVec, wp%opt%gridOrigin,&
        & totChrg, fileName, comments=comments, repeatBox=wp%opt%repeatBox)
    write(stdOut, "(A)") "File '" // trim(fileName) // "' written"
    if (wp%opt%tVerbose) then
...
contains
  ...
  #:endblock DEBUG_CODE
    ...
    do ii = 1, geo%nAtom
      write(fd, "(I5,4F12.6)") atomicNumbers(geo%species(ii)), 0.0_dp, geo%coords(:, ii)
    end do
