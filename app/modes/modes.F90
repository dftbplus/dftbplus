...
program modes
  ...
  use modes_initmodes, only : dynMatrix, bornMatrix, modesToPlot, geo, iMovedAtoms, nCycles,&
      & nDerivs, nModesToPlot, nMovedAtom, nSteps, tAnimateModes, tPlotModes, tEigenVectors,&
      & tRemoveRotate, tRemoveTranslate, atomicMasses, initProgramVariables
...
#:endif
  ...
  if (allocated(bornMatrix)) then
    ...
    nTrans = 1
    do jj = 2, nDerivs
      ! test for degeneracy to printing cutoff:
  ...
  if (allocated(bornMatrix)) then
    write(stdout, "(T7,A,T16,A)")'freq.','trans. dip.'
