...
DFTBPLUS_PARAM_DIR = os.environ['DFTBPLUS_PARAM_DIR']
...
class HsdinputTest(common.TestWithWorkDir):
    ...
    def test_maxang_try_reading_dict(self):
        ...
        hsd_dict = Hsdinput(hamiltonian="DFTB")
        path1 = os.path.join(DFTBPLUS_PARAM_DIR, "slakos/origin/mio-1-1")
        path2 = os.path.join(DFTBPLUS_PARAM_DIR, "slakos/origin/3ob-3-1")
        skdir_dict = {'O-O': f'{path1}/O-O.skf',
                      "O-H": f"{path1}/O-H.skf",
                      'H-O': f'{path1}/H-O.skf',
                      "H-H": f"{path1}/H-H.skf",
                      "Br-Br": f"{path2}/Br-Br.skf"}
        hsd_dict.set_skdir(skdir_dict)
    ...
    def test_maxang_try_reading_str(self):
        ...
        hsd_dict = Hsdinput(hamiltonian="DFTB")
        skf_path = os.path.join(DFTBPLUS_PARAM_DIR, "slakos/origin/mio-1-1")
        hsd_dict.set_skdir(skf_path)
        maxangs = {"Br": '"d"'}
        hsd_dict.set_maxang(maxangs=maxangs, try_reading=["O", "H"])
        path = os.path.join(self.inputdir, "maxang_try_reading_ref.hsd")
        reference = Hsdinput(filename=path, hamiltonian="DFTB")
        reference.set_skdir(skf_path)
        self.assertTrue(common.type_diff(reference.get_hsd(),
                                         hsd_dict.get_hsd()))
