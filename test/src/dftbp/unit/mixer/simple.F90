...
contains
  ...
  $:TEST("simple_mixer_mix1D")
    ...
    real(dp) :: mixParam = 0.5_dp

    real(dp) :: charges(3) = [0.0_dp, 1.0_dp, 2.0_dp]
    real(dp) :: chargeDiff(3)  = [1.0_dp, 0.0_dp, 2.0_dp]
    real(dp) :: expected(3) = [0.5_dp, 1.0_dp, 3.0_dp]

    ! Initialize the mixer
    mixerinp%mixParam = mixParam
    call TSimpleMixerReal_init(mixer, mixerinp)
    ! Prepare; (currently) without effect for the simple mixer since its stateless
    call mixer%reset(3)
  ...
  $:TEST("simple_mixer_mix3d")
    ...
    real(dp), parameter :: atol = 100.0_dp * epsilon(0.0_dp)
    real(dp) :: mixParam = 0.5_dp
    ! Since these matrices are quite unwieldy,
    ! we initialise them to 0 and just add a few markers values here and there.
    real(dp) :: charges(3, 3, 3) = 0.0_dp
    real(dp) :: chargeDiff(3, 3, 3) = 0.0_dp
    real(dp) :: expected(3, 3, 3) = 0.0_dp
  ...
  $:TEST("simple_mixer_mix6d")
    ...
    real(dp) :: charges(3, 3, 3, 3, 3, 3) = 0.0_dp
    real(dp) :: chargeDiff(3, 3, 3, 3, 3, 3) = 2.0_dp
    real(dp) :: expected(3, 3, 3, 3, 3, 3) = 1.0_dp
