...
contains
  ...
  $:TEST("anderson_mixer_H2O")
    ...
    real(dp) :: expectedCharges(3, 5) = reshape([ &
    ...
    real(dp) :: difference(3, 4) = reshape([ &
    ...
    do i = 1, 4
        call mixer%mix(qInpResult, difference(:,i))
        @:ASSERT(all_close(qInpResult, expectedCharges(:,i+1), atol=atol))
    end do
  $:END_TEST()
