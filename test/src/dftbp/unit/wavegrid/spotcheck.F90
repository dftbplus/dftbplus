...
module test_wavegrid_spotcheck
  ...
  !> All 3 combinations of CPU using lookup table/Direct calculation and GPU using lookup table.
  #:set configCount = 3 if WITH_CUDA else 2
  type(TLaunchConfig), parameter :: launchConfigs(${configCount}$) = [ &
    #:if WITH_CUDA
      TLaunchConfig(.true.,  .true.), & ! Lookup table GPU
    #:endif
      TLaunchConfig(.false., .false.), & ! Direct CPU
      TLaunchConfig(.true.,  .false.) & ! Lookup table CPU
  ]
  ...
  !> Allow 0.1% relative error (lookup table interpolation etc.)
...
end module test_wavegrid_spotcheck
