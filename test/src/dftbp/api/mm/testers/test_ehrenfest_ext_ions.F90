...
contains
  ...
  subroutine main(errStatus)
    ...
    call setChild(pRoot, "ParserOptions", pParserOpt, errStatus)
    @:PROPAGATE_ERROR(errStatus)
    call setChildValue(pParserOpt, "ParserVersion", 13, errStatus)
    @:PROPAGATE_ERROR(errStatus)
    ...
    @:PROPAGATE_ERROR(errStatus)
    call setChildValue(pElecDyn, "IonDynamics", .true., errStatus)
    @:PROPAGATE_ERROR(errStatus)
    call setChildValue(pElecDyn, "InitialTemperature", 0.0_dp, errStatus)
    @:PROPAGATE_ERROR(errStatus)

    call setChild(pElecDyn, "Perturbation", pPerturb, errStatus)
    @:PROPAGATE_ERROR(errStatus)
    call setChild(pPerturb, "Laser", pLaser, errStatus)
    @:PROPAGATE_ERROR(errStatus)
    ! these twovalues will be overriden
    call setChildValue(pLaser, "PolarisationDirection", [0.0_dp, 1.0_dp, 1.0_dp], errStatus)
    @:PROPAGATE_ERROR(errStatus)
    call setChildValue(pLaser, "LaserEnergy", 1.0_dp, errStatus)
    @:PROPAGATE_ERROR(errStatus)
