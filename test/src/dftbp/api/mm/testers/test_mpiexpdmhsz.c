...
typedef double complex hs_type;
...
void gather(int *blacs_descr, const void *blacs_data, void *dest) {
  ...
  int ONE = 1;
  pzgemr2d_(&blacs_descr[M_], &blacs_descr[N_], blacs_data, &ONE, &ONE,
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < basis_size; ++i)
    for (int j = 0; j < basis_size; ++j) {
      hs_type d = conj(dm[i*basis_size+j]);
      n_el += overlap[i*basis_size+j] * d;
      eisum += hamiltonian[i*basis_size+j] * d;
    }
  ...
  for (int i = 0; i  < world_size; ++i)
  {
    ...
    if (i == world_rank)
    {
      printf("local_ks %d = %d %d\n", i, local_ks[0], local_ks[1]);
      printf("n_el %d = %f\n", i, n_el);
      printf("eisum %d = %f\n", i, eisum);
      fflush(stdout);
    }
  ...
  double total_n_el = 0, total_eisum = 0;
  MPI_Reduce(&n_el, &total_n_el, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&eisum, &total_eisum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
