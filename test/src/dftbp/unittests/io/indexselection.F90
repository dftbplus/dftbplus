...
#:contains
  ...
  #:contains
    ...
    #:block TEST("explicit")
      call getIndexSelection("1 2 4", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
    ...
    #:block TEST("allAsRange")
      call getIndexSelection("1:-1", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(.not. allocated(error))
      @:ASSERT(all(selected))
    #:endblock
    ...
    #:block TEST("range")
      call getIndexSelection("2:6", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("negativeSubrange")
      call getIndexSelection("-6:-2", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("speciesName")
      call getIndexSelection("Si", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("excludeIndices")
      call getIndexSelection("!4:14", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("excludeSpecies")
      call getIndexSelection("!C", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(.not. allocated(error))
      @:ASSERT(checkSelection(selected, fullRange, [1, 3, 5, 7, 9, 11, 13, 15]))
    #:endblock
    ...
    #:block TEST("excludesWithAnd")
      call getIndexSelection("!Si & !1:4 & !12:16", fullRange, selected, error,&
    ...
    #:block TEST("andOrPrecedence")
      call getIndexSelection("1:3 6:8 & Si 9:12", fullRange, selected, error,&
           speciesNames=speciesNames, species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("parenthesisPrecedence2")
      call getIndexSelection("1 (1:3 6:8) & !Si 9:12", fullRange, selected, error,&
          speciesNames=speciesNames, species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("zeroIndexingPositive")
      call getIndexSelection("0 2 3", [0, nAtom - 1], selected, error,&
          speciesNames=speciesNames, species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("zeroIndexingNegative")
      call getIndexSelection("-16:-14", [0, nAtom - 1], selected, error,&
          speciesNames=speciesNames, species=species)
      @:ASSERT(.not. allocated(error))
    ...
    #:block TEST("subrange")
      call getIndexSelection("11:13", [9, 16], selected(9:16), error,&
    ...
    #:block TEST("selectWrongSpeciesName")
      call getIndexSelection("Mg", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(allocated(error))
      @:ASSERT(error%code == errors%invalidSelector)
      call error%deactivate()
    #:endblock

    #:block TEST("outOfBounds")
      call getIndexSelection("17", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(allocated(error))
      @:ASSERT(error%code == errors%invalidSelector)
      call error%deactivate()
    ...
    #:block TEST("outOfBoundsPlusZeroInd")
      call getIndexSelection("16", [0, nAtom - 1], selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(allocated(error))
      @:ASSERT(error%code == errors%invalidSelector)
      call error%deactivate()
    ...
    #:block TEST("outOfSubrangeBounds")
      call getIndexSelection("1:10", [9, 16], selected(9:16), error,&
          & speciesNames=speciesNames, species=species(9:16), indexRange=fullRange)
      @:ASSERT(allocated(error))
      @:ASSERT(error%code == errors%invalidSelector)
      call error%deactivate()
    ...
    #:block TEST("doubleOperator")
      call getIndexSelection("1 && 2", fullRange, selected, error, speciesNames=speciesNames,&
          & species=species)
      @:ASSERT(allocated(error))
      @:ASSERT(error%code == errors%syntaxError)
      call error%deactivate()
    #:endblock
  ...
  #:contains
     ...
     #:block TEST("explicit")
       call getIndexSelection("5 6 7", fullRange, selected, error)
       @:ASSERT(.not. allocated(error))
       @:ASSERT(checkSelection(selected, fullRange, [5, 6, 7]))
      #:endblock

      #:block TEST("range")
        call getIndexSelection("-4:-2", fullRange, selected, error)
        @:ASSERT(.not. allocated(error))
  ...
  #:contains
     ...
     #:block TEST("explicit")
       call getIndexSelection("-1 0 1", fullRange, selected, error)
       @:ASSERT(.not. allocated(error))
       @:ASSERT(checkSelection(selected, fullRange, [-1, 0, 1]))
      #:endblock

      #:block TEST("range")
        call getIndexSelection("-1:1", fullRange, selected, error)
        @:ASSERT(.not. allocated(error))
        @:ASSERT(checkSelection(selected, fullRange, [-1, 0, 1]))
       #:endblock
