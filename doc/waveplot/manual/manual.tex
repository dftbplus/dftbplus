...
\title{\textbf{\waveplot: An orbital visualisation tool\\\ \\Manual}}
\date{}
\author{Bálint Aradi}
...
\chapter{Input for \waveplot}

The input file for \waveplot{} must be named \verb|waveplot_in.hsd|
and should be a Human-friendly Structured Data (HSD) formatted
file.\footnote{ For details of the HSD format, please consult the
manual of the \textsc{DFTB$^{\text{+}}$} code at
...
file is \verb|waveplot_in.xml|. The input file must be present in the
working directory. To prevent ambiguity, the parser refuses to read in
any input if both files are present.

The input file for {\waveplot} must contain the properties listed in
...
\begin{ptableh}
  \kw{Options} & p &   & - &\pref{Options} \\
  \kw{DetailedXML} &s &  & - &  \\
  \kw{EigenvecBin} &s &  & - &   \\
  \kw{GroundState} &s &  & Yes &   \\
  \kw{Basis} & p & & - &  \pref{Basis} \\
\end{ptableh}
\begin{description}
\item[\is{Options}] Contains the options for \waveplot{}. See p.~\pref{Options}.
\item[\is{DetailedXML}] Specifies the name of the file, which contains the
  ...
  \is{detailed.xml}).
\item[\is{EigenvecBin}]\label{EigenvecBin} Specifies the name of the file, which
  contains the eigenvectors in binary format (presumably \is{eigenvec.bin}).
\item[\is{GroundState}]\label{GroundState} Read ground or excited state
  occupation data from the detailed XML output.
...
\begin{ptableh}
  \kw{ParserOptions} & p & & \cb & \pref{ParserOptions} \\
\end{ptableh}
...
\begin{ptable}
  \kw{PlottedRegion} &p|m&  & - &\pref{PlottedRegion}\\
  \kw{NrOfPoints} & 3i & & - &  \\
...
\item[\is{PlottedRegion}] Regulates the region which should be
  plotted. See p.~\pref{PlottedRegion}.
...
it automatically using either the \islcb{UnitCell} or the
\islcb{OptimalCuboid} methods.
...
\htcbsubsubsection{OptimalCuboid}
...
\begin{ptable}
  \kw{Resolution} & r &  & - &  \\
  \textit{ElementName1} & p & & - & \pref{speciesBasis} \\
  \textit{ElementName2} & p & & - &  \pref{speciesBasis} \\
  \hspace*{0.8cm}$\vdots$ & & & & \\
\end{ptable}
\begin{description}
...
\subsection{Basis for an atom type}
\label{speciesBasis}
