...
The expressions might contain the logical operators ``not'' (\verb|!|), ``and'' (\verb|&|) and
``or'' (space). The ``not'' operator has the highest and the ``or'' operator the lowest precedence.
Parentheses can be used for grouping operations. If atoms are being selected, atom type names can be
used to select all atoms of a given type. The first element of the list has the index one. Negativ
indices are counted from backwards from the last element of the list, with -1 corresponding to the
last element. Index ranges can be specified as \verb|start:end| (without white space as one word!),
which inclusively selects all elements between \verb|start| and \verb|end|.
...
# All atoms from 2 to 6 which are Si atoms
MovedAtoms = 2:6 & Si

# All atoms, apart of atoms 2, 3, 4, 5, 6
MovedAtoms = !2:6
...
# All non-Si atoms in the ranges [1, 10], [21, 30] and [41, 50].
