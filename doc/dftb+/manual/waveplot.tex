...
\item[\is{UseGpu}] If true, use CUDA GPU parallelisation.
  The calculation uses single-precision texture interpolation.
  Requires a compatible GPU and a \verb|WITH_CUDA=TRUE| build.
  This options is requires the usage of at most one MPI process.
...
\begin{description}
\item[\is{Type}] Radial orbital type. Currently only \is{TSlaterOrbital} is
  supported.
