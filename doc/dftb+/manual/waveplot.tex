...
as part of the input file. Alternatively the tabulated radial wavefunction
for different ranges and its second derivative can be entered. In this
case the radial wavefunctions will be calculated via interpolation.
...
\begin{ptable}
  ...
  \kw{ShiftGrid} & l & & Yes & \\
  \kw{RadialWFTabulation} & s & & explicit & \\
  \kw{GridInterpolation} & s & & linear & \\
  \kw{ParallelRegionNum} & i & & number of threads & \\
  \hline
\end{ptable}
\begin{description}
\item[\is{PlottedRegion}] Regulates the region which should be
  plotted. See p.~\pref{sec:waveplot.PlottedRegion}.
\item[\is{TotGridPoints}] Specifies the resolution of the equidistant grid on
...
Example:
\begin{verbatim}
TotGridPoints = { 50 50 50 }     # 125 000 grid points
\end{verbatim}

\item[\is{SpGridPoints}] Same functionality as "TotGridPoints". Specifies the
  resolution of the equidistant grid on which the Slater type orbitals are
  tabulated before they get mapped onto the total grid. The species grids will
  not be used if \is{GridInterpolation} is set to "explicit".
...
\item[\is{RadialWFTabulation}] Specifies the method which should be used for the
  tabulation of the radial wavefunctions of the Slater type orbitals. Available
  options are
  \begin{itemize}
	  \item{explicit} Calculates the radial wavefunctions explicitly with Eq.
	  	(\ref{eq:raddef})
      \item{trivial} Interpolates the radial wavefunctions from an pre-tabulated
      	file with trivial interpolation
      \item{linear} Interpolates the radial wavefunctions from an pre-tabulated
      	file with linear interpolation
      \item{spline} Interpolates the radial wavefunctions from an pre-tabulated
      	file with spline interpolation
  \end{itemize}
  If "explicit" is specified you need to provide an file with the coefficients and
  exponents for the Slater type orbitals. If one of the interpolation types is
  specified, you need to provide an file with the tabulated radial wavefunction
  and its second derivative.

\item[\is{GridInterpolation}] Specifies the 3d interpolation method which should
	be used for the mapping of the Slater type orbitals from the species grids onto
	the total grid. Available options are
	\begin{itemize}
	  \item{trivial} Uses trivial interpolation for the grid mapping
    \item{linear} Uses linear interpolation for the grid mapping
  \end{itemize}

\item[\is{ParallelRegionNum}] During the grid mapping, the total grid is divided
	into "ParallelRegionNum" grid slices. OpenMP is used to speed up the calculation
	and calculate the grid slices with different threads. The default is the number
	of threads which your machine offers.
...
\item[\is{RadialWFTabulation}] Specifies the method which should be used for the
  ...
  \begin{itemize}
	  \item{explicit} Calculates the radial wavefunctions explicitly with Eq.
	  	(\ref{eq:raddef})
      \item{trivial} Interpolates the radial wavefunctions from a pre-tabulated
      	file with trivial interpolation
      \item{linear} Interpolates the radial wavefunctions from a pre-tabulated
      	file with linear interpolation
      \item{polynomial} Interpolates the radial wavefunctions from a pre-tabulated
      	file with polynomial interpolation
  \end{itemize}
  If "explicit" is specified you need to provide a file with the coefficients and
  exponents for the Slater type orbitals. If one of the interpolation types is
  specified, you need to provide a file with the tabulated radial wavefunction
  and its second derivative.
...
\item[\is{ParallelRegionNum}] During the grid mapping, the total grid is divided
	into \is{ParallelRegionNum} grid slices. OpenMP is used to speed up the calculation
	and calculate the grid slices with different threads. The default is the number
	of threads which your machine offers.
...
The basis definition which uses the coefficients and exponents for an explicit
calculation of the radial wavefunctions can be used if \is{RadialWFTabulation}
in the options block is set to "explicit". Then the basis definition is done
by specifying the following properties:

\begin{ptable}
  \textit{ElementName1} & p & RadialWFTabulation = explicit & - &
  	\pref{sec:waveplot.speciesBasis} \\
  \textit{ElementName2} & p & RadialWFTabulation = explicit & - &  \pref{sec:waveplot.speciesBasis} \\
  \hspace*{0.8cm}$\vdots$ & & & & \\
\end{ptable}
\begin{description}
...
Alternatively, the pre-tabulated radial wavefunction and its derivatives has to
be provided, if one of the interpolation types is specified for \is{RadialWFTabulation}.
The Basis definition is similar to the previous case, but the radial wavefunction is
tabulated instead of the coefficients and exponents. As an example, the \is{Basis}
...
Basis = {
  H = {
    ...
    Orbital = {
      ...
      Cutoff = 5.0
      RadialWaveFunc = {
        9.830308526563000292e-06 2.475712095616000141e+00 3.091155745115000020e+00
        ...
      }
    }
  }
  O = {
    ...
    Orbital = {
      ...
      Cutoff = 5.0
      RadialWaveFunc = {
        9.830308526563000292e-06 -1.059886964684999988e+01 -6.025328228465999700e+02
        ...
      }
    }
  	Orbital = {
      ...
      Cutoff = 5.0
      RadialWaveFunc = {
        9.830308526563000292e-06 1.622019064264999940e-04 -1.238026858581000056e+02
        ...
      }
    }
  }
