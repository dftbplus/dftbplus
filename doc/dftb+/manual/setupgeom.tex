...
\begin{ptable}
 ...
 \kw{ContactVector} & 3r & & - & \\	
 \kw{FoldToUnitCell} & l & & No &\\ 
 \kw{Translation} & 3r & & 0.0 &\\ 
...
\item[\is{ContactVector}] Sets the translation vector connecting the 2 PLs along the transport direction. 
	Since contact must be aligned to a cartesian axis, so must be this vector. Different contact
      ...
      direction.
\item[\is{TruncateSKRange}]  This section is the same as that described in section \ref{sec:dftbp.Hamiltoanian}.
...
\item[\is{FoldToUnitCell}] Is used to fold all coordinates to the unit cell such that all fractional
...
\item[\is{Translation}] Adds a rigid translation vector to the atomic coordinates  
