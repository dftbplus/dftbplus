...
\item[\is{FirstLayerAtoms}] defines the first atom of PLs in the device
  ...
  the structure into distinc layers for more efficient Green's function
  calculations.

  The device layers, unlike the contact PLs, do not need to represent unit cell
  repetitions. If the device geometry has specified princpal layers, these must
...
\item[\is{FermiLevel}]\modif{\modtype{energy}} Specifies the Fermi level of the
  contact.
\item[\is{Potential}]\modif{\modtype{energy}} Specifies any electrostatic
...
perpendicular to the transport direction should be accurate, but the grid can
have only a single value in the transport direction.
...
\item[\is{Delta}]\modif{\modtype{energy}}\modif{\modtype{energy}} A small
  positive imaginary delta used in the GF definition and required for the
  contour integration.
  \item[\is{ContourPoints}] The number of points along the complex contour
    ...
    (see contour integration in section~\ref{sec:transport.contourint}).
...
\item[\is{BoxExtension}]\modif{\modtype{length}} With this value it is possible 
  ...
  By default (\is{BoxExtension}=0.0) the boundary is placed on the midpoint
...
\item[\is{AtomDensityCutoff}]\modif{\modtype{length}}  Defines the atomic radius
  cutoff. This is alternative to \is{AtomDensityTolerance}.
...
\item[\is{SavePotential}] Save the potential to the file \verb|potential.dat| and 
...
can use the parameter \kw{MaxParallelNodes} to control distributions of these
...
\item[\is{ContactTemperature}]\modif{\modtype{energy}} Specifies the electronic
  temperature of the contacts for the calculation of currents. It expects an
  array of reals, one value per contact following the order given in \is{Transport}.
...
In the current state of the code the electronic temperature of the system 
can be set in different places. One place is within the Hamiltonian block
in the \is{Filling} section. The Temperature specified here is effective 
for the whole device and applies to contact calculations as well. 
Note that during contact calculations the temperature is read from 
the \is{Filling} section and *not* from the contact section. 
During contact calculations only Fermi filling make sense.

When a temperature is specified in the \is{Contact} section of the \is{Transport} 
block, it overrides the system temperature specified in \is{filling}.

\bf{Note} that the current behaviour is going to be changed soon. 

It is also possible to specify a (different) temperature in the section
\is{TunnelingAndDOS}, within the block \is{Analysis}. The latter applies only 
in the calculation of currents (integration of the transmission function).

Although slightly inconsistent, in some cases it is useful to set a somewhat 
larger temperature in the calculation of the density as it helps convergence 
of the self-consistent loop, but compute currents with a different temperature.
