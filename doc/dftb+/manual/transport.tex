...
\item[\iscb{ButtikerProbes}] \label{Buttiker} This block is used to specify B\:uttiker 
	probes. Buttiker Probes can be either of type \is{ZeroPotential} or ...
...
\item[\is{Coupling}] is the system/probe coupling energy.
   	


\item[\iscb{VibronicElastic}] \label{elastic} This block is used to specify the
	elastic dephasing model. This can be \is{elastic} or ...

\item[\is{Coupling}] is the electron-phonon coupling energy. \is{Coupling} can be 
	\is{Constant}, uniform on the whole device region or can be defined for 
	each atom using the key \is{AtomCoupling} and \is{AtomList}. Example:
