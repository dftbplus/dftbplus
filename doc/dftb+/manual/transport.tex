...
{\bf Note}: Currently the code makes a consistency check on the definition of the
...
The Transport section groups the information needed whenever open boundary
...
The \is{GreensFunction} block is used to solve the self-consistent NEGF 
transport problem. However, the block \iscb{TunnelingAndDos} within the 
\is{Analysis} block (see below),  
...
should be omitted (i.e.\ the \is{Electrostatics = Poisson}).
The keyword \is{Eigensolver = TransportOnly} is used to jump straight to the 
post-SCF analysis.
