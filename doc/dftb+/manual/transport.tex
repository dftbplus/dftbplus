...
Green's function solver must be used. However, the \iscb{TunelingAndDos} section
...
calculation, \is{Eigensolver = TransportOnly} must be set and no electrostatics
must be calculated (i.e.\ the \is{Electrostatics = Poisson} should not be
declared).
...
\item[\is{SavePotential}] Save the potential to the file \verb|potential.dat| and 
  ...
  can be visualized in jmol. See section on transport tools.
...
This program can be used to convert the real-space  'potential.dat' or 
'charge_density.dat' files computed on the poisson box to a cube file that can 
be plotted using jmol. 
